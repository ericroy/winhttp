...
namespace http
{
	...
	{
		const std::wstring &wurl = req.url();
		...
		auto headersEnd = std::end(req.additional_headers());
		for(auto iter = std::begin(req.additional_headers()); iter != headersEnd; ++iter) {
		...
		DWORD total_request_length = req.body().length();
		...
		if(total_request_length > 0) {
			...
			if(!WinHttpWriteData(request, req.body().data(), req.body().length(), &bytes_written)) {
				THROW_LAST_ERROR("WinHttpWriteData() failed");
				return response(nullptr);
			}
			if(bytes_written != req.body().length()) {
				THROW_ERROR("WinHttpWriteData did not send entire request body");
				return response(nullptr);
			}
		}
